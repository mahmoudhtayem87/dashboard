...
export class CheckboxComponent implements AfterViewInit {
  ...
  prepareDataSet() {
    ...
    var dataItemYes = new ChartDataItem();
    dataItemYes.key = "true";
    dataItemYes.label = "Yes";
    dataItemYes.frequancy = 0;
    this.dataSet.push(dataItemYes);
    var dataItemNo = new ChartDataItem();
    dataItemNo.key = "false";
    dataItemNo.label = "No";
    dataItemNo.frequancy = 0;
    this.dataSet.push(dataItemNo);

    var data = this.structure.data;

    for (let dataElementIndex = 0; dataElementIndex < data.length; dataElementIndex++) {
      var item = data[dataElementIndex].value;
